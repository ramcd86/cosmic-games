...
import { ClientToServerEvents, ServerToClientEvents, GameAction } from '@cosmic-games/shared';
import { GameManager } from '../services/GameManager';

export class SocketHandler {
  private io: Server<ClientToServerEvents, ServerToClientEvents>;
  private gameManager: GameManager;
  private playerSessions: Map<string, { playerId: string; playerName: string; roomCode: string }> = new Map();
    ...
    this.io.on('connection', (socket) => {
      ...
      socket.on('join-room', async (roomCode: string, playerName: string) => {
        try {
          ...
          const player = result.room.players.find(p => 
            p.name === playerName && !p.isAI
          );
      ...
      // Chat message event
      socket.on('chat-message', async (roomCode: string, message: string) => {
        try {
          const session = this.playerSessions.get(socket.id);
          if (!session) return;

          // Broadcast chat message to all players in the room
          this.io.to(roomCode).emit('chat-received', 
            session.playerId, 
            session.playerName, 
            message, 
            new Date()
